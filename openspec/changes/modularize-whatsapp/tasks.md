...
- [ ] 2.3 Move messaging/media/reaction code into `messaging/{mod.rs,send.rs,reactions.rs,media.rs,preview.rs}`. _(PR3g extracted media download/proxy/cache; template-header upload and inbound media-failure fallback still remain in `handler.rs`.)_
