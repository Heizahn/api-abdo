...
- [ ] 2.3 Move messaging/media/reaction code into `messaging/{mod.rs,send.rs,reactions.rs,media.rs,preview.rs}`. _(PR3h extracted template-header upload; inbound media-failure fallback still remains in `handler.rs`.)_
