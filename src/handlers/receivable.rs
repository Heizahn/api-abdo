...
pub async fn me_receivables_handler(
    ...
    // 3. Obtener Deudas Activas (Según BD)
    let debts = state
        .db
    ...
    let epsilon = 0.001; // Tolerancia para punto flotante
    ...
    for debt in debts {
        ...
        for pp in debt_parts {
            if let Some(payment) = processed_payments.iter().find(|p| p._id == pp.id_payment) {
                // CORRECCIÓN: Case insensitive ("Activo" == "activo" == "ACTIVO")
        ...
        // CORRECCIÓN: Redondeo matemático antes de la resta para evitar basura
        ...
        // Si es negativo (sobrepago) o muy cercano a 0, lo forzamos a 0.0
        let pending_usd = if pending_usd_raw < epsilon { 0.0 } else { pending_usd_raw };

        // Mostrar deuda SI hay saldo > 0 O SI hay reportes pendientes visuales
