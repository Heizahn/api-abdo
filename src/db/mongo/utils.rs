...
impl UtilsRepository for MongoDB {
    ...
    async fn find_client_olt_position(&self, client_id: &str) -> Result<(String, String), ApiError> {
        ...
        let mut cursor = collection.aggregate(pipeline).await.map_err(|e| {
            ApiError::DatabaseError(format!("Error en consulta a la BD: {}", e))
        })?;

        // 4. Leer el primer resultado del cursor
        if let Some(doc) = cursor.try_next().await.map_err(|_| ApiError::DatabaseError("Error al leer datos".to_string()))? {

            // Deserializamos el documento BSON a nuestra estructura de Rust
            let res: ZabbixLookupResult = mongodb::bson::from_document(doc).map_err(|_| {
                ApiError::Internal("Error al parsear datos de la BD".to_string())
            })?;
