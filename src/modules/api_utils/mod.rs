...
pub fn public_routes() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route(
            ...
            get(handler::get_latest_version_response),
        )
}
...
pub fn user_routes() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route(
            "/v1/auth-user/utils/list/banks",
            get(handler::get_bank_list_user),
        )
}

/// Rutas protegidas con JWT de cliente
pub fn client_routes() -> Router<Arc<AppState>> {
    Router::new().route("/v1/utils/list/banks", get(handler::get_bank_list))
}

/// Rutas estáticas (sin auth)
pub fn static_routes() -> Router<Arc<AppState>> {
    Router::new().route("/v1/privacy-policy", get(handler::get_privacy_policy))
}
